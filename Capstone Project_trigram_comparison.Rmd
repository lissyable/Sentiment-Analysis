...
TD_trigram_top1 <- TD_trigram_counts %>% arrange(desc(n))%>% head(20)%>%ggplot(aes(x=factor(trigram,levels=trigram),y=n))+geom_bar(stat="identity",fill="#FF3E45")+labs(title="Top 20 trigram words in Product")+coord_flip()

wordcloud(TD_trigram_counts$trigram, TD_trigram_counts$n, max.words=50, random.order = F, colors=c("#9E0142", "#D53E4F", "#F46D43" ,"#FDAE61", "#FEE08B", "#E6F598" ,"#ABDDA4" ,"#66C2A5" ,"#3288BD", "#5E4FA2"), scale = c(1, 0.4))
...
TD_trigram_top2 <- TD_trigram_counts %>% arrange(desc(n))%>% head(20)%>%ggplot(aes(x=factor(trigram,levels=trigram),y=n))+geom_bar(stat="identity",fill="#FF3E45")+labs(title="Top 20 trigram words in Product")+coord_flip()
...
SCOTIA_trigram_top1 <- SCOTIA_trigram_counts %>% arrange(desc(n))%>% head(20)%>%ggplot(aes(x=factor(trigram,levels=trigram),y=n))+geom_bar(stat="identity",fill="#FF3E45")+labs(title="Top 20 trigram words in Product")+coord_flip()
...
```
# lowered capital markets, hold rating capital
